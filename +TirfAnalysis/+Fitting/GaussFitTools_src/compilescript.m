...
mex ('-I..','-O', '-L..', '-L..', file{iFile},...
    '-llevmar', '-lLAPACK.lib', '-lclapack.lib',...
    '-lBLAS.lib', '-lBLAS_nowrap.lib', '-llibf2c.lib',...
    '-lf77blaswrap.lib', '/NODEFAULTLIB:libcmt.lib');
end
...
%   movefile(['*',outfileType,'*'],binFolder);
%   copyfile(['*.m'],binFolder);
% elseif strcmp(ostype,'MACI64') %Mac 64bit install
%   FLAGS_64BIT='-fPIC';
%   binFolder='GaussFitTools_bin_Mac64';
% 
%   for i = 1:numel(file)
%     mex('-O',['CFLAGS=',FLAGS_64BIT],['-L',binFolder], '-llevmar',file{i}, '-llapack' ,'-lblas')
%   end
%   outfileType = 'mexmaci64';
%   movefile(['*',outfileType,'*'],binFolder);
%   copyfile(['*.m'],binFolder);
% 
% end
